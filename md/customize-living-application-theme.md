...
The `artifactId`, `name` and `description` are used to defined the theme metadata (the name used in the URL, the display name, and a description) in the `page.properties`.  

NodeJS and NPM version are fixed in the `properties` section. The `frontend-maven-plugin` will locally install and use these versions even if you have  NodeJS and NPM already installed in your environment. See the [plugin github repository](https://github.com/eirslett/frontend-maven-plugin) for more informations.
...
* `@import "bonita_variables";` import the content of `_bonita_variables.scss` file.
